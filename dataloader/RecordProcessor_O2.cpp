...
int RecordProcessor::ProcessO2Record(array<Byte>^ record) {
    try {
        ...
        command->CommandText = "SELECT creation_date FROM umaren WHERE "
        ...
        if (existingDateObj != nullptr) {
            ...
            command->CommandText = "DELETE FROM umaren WHERE "
                "kaisai_date = @kaisai_date AND "
                "keibajo_code = @keibajo_code AND "
                "kaisai_kai = @kaisai_kai AND "
                "kaisai_nichime = @kaisai_nichime AND "
        ...
        for (int i = 0; i < 153; i++) {
            Int16 umaban1;
            try {
                umaban1 = Int16::Parse(ByteSubstring(record, 40 + (i * 13), 2));
            }
            catch (...) {
            ...
            Int16 umaban2;
            try {
                umaban2 = Int16::Parse(ByteSubstring(record, 40 + (i * 13) + 2, 2));
            }
            catch (...) {
                continue;  // Skip if we can't parse a valid umaban (likely empty/padding)
            }
            ...
            String^ oddsValue;
            try {
                Int32 odds = Int32::Parse(ByteSubstring(record, 40 + (i * 13) + 4, 6));
        ...
        Console::WriteLine();
        Console::WriteLine("Failed to process O2 record: {0}", ex->Message);
