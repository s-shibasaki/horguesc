...
int RecordProcessor::ProcessO3Record(array<Byte>^ record) {
    try {
        ...
        command->CommandText = "SELECT creation_date FROM wide WHERE "
        ...
        if (existingDateObj != nullptr) {
            ...
            command->CommandText = "DELETE FROM wide WHERE "
                "kaisai_date = @kaisai_date AND "
                "keibajo_code = @keibajo_code AND "
                "kaisai_kai = @kaisai_kai AND "
                "kaisai_nichime = @kaisai_nichime AND "
        ...
        Console::WriteLine();
        Console::WriteLine("Failed to process O3 record: {0}", ex->Message);
