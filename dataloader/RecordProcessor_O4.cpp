...
int RecordProcessor::ProcessO4Record(array<Byte>^ record) {
    try {
        ...
        command->CommandText = "SELECT creation_date FROM umatan WHERE "
        ...
        if (existingDateObj != nullptr) {
            ...
            command->CommandText = "DELETE FROM umatan WHERE "
                "kaisai_date = @kaisai_date AND "
                "keibajo_code = @keibajo_code AND "
                "kaisai_kai = @kaisai_kai AND "
                "kaisai_nichime = @kaisai_nichime AND "
        ...
        if (batchValues->Count > 0) {
            ...
            command->CommandText = "INSERT INTO umatan (data_type, creation_date, "
        ...
        Console::WriteLine();
        Console::WriteLine("Failed to process O4 record: {0}", ex->Message);
